# Ведомость СЧ ОКР
> 2019.05.12 ┊ **🚀 [despace](index.md)** → [КД](doc.md#КД), [НД](doc.md), [R&D](rnd.md)
...
**Ведомость** — документ, включающий в себя перечень разработанной в результате аванпроекта (эскизного проекта, технического проекта) [конструкторской документации](doc.md#КД).
...
В соответствии с [СТО 301-36](sto_301_36.md) Ведомость:  
*Согласовывается:* Исполнителем, СОК, [ГКП](rnd_mgmt.md), РКП, Зам. ГК, 4116 ВП.  
*Утверждается:* ГК.

**Типичные [конструкторские документы](doc.md#КД), входящие в Ведомость на указанных выше этапах:**
...
## Длинное описание
Ведомость АП, ЭП и ТП составляют на формах, представленные ниже.   В АП, ЭП и ТП записывают все [конструкторские документы](doc.md#КД), вновь разработанные для данного АП, ЭП и ТП и примененные из других проектов и рабочей документации на ранее разработанные изделия. При этом записывают только те документы, которые являются необходимыми и достаточными для рассмотрения и утверждения данного проекта.
...
Ведомость оценивается по [тарифу №2](03_lav.md).
...
**Docs:**
   ...
   1. Ведомость АП. [Шаблон ❐](f/doc/vedomost_ap_template.doc).
   1. Ведомость ЭП. [Пример ❐](f/doc/vedomost_sample.pdf). [Шаблон ❐](f/doc/vedomost_template.doc).
