...
**Разработка конструкторской документации (РКД), разработка рабочей документации (РРД).** Рассмотрение и утверждение [ТП](rnd_tp.md) ([ЭП](rnd_ep.md), если не было ТП). Разработка [КД](doc.md#КД), предназначенной для изготовления и [испытания](test.md) опытного образца (партии), без присвоения литеры.
...
Работы на этапе РКД (РРД) проводятся в целях:
   ...
   - разработки [ТД](doc.md#ТД) для изготовления опытных образцов и макетов и программ (планов) подготовки производства;
...
|*Наименование этапа РКД (РРД)*|*Отв.*|*Длит.,<br> р.д.*|
|:--|:--|:--|
|**1. Получение контракта на РКД (РРД)**  |  —  |**21**  |
|·   1.1 Заключение контракта  |  Дир.  |   20  |
|·   1.2 Представление контракта и ТЗ [ГКП](rnd_mgmt.md)  |  Дир.  |   1  |
|**2. Подготовка к разработке РКД (РРД)** *(параллельно)*  |  —  |**25**  |
|·   2.1 Разработка [МС](matrix_compl.md) на КК  |  ГКП  |   2  |
|·   2.2 Разработка организационных документов *(параллельно)*  |  —  |   23  |
|··     2.2.1 Формирование ИД  |  СГК  |   23  |
|··     2.2.2 Формирование графика работ  |  Дир.  |   20  |
|··     2.2.3 Отправка в НПОЛ решения о порядке экспертизы  |  Заказчик  |   5  |
|··     2.2.4 Формирование [РГ](wg.md), назначение [ГКП, РПР, МК, ГТ](rnd_mgmt.md)  |  Дир.  |      20  |
|**3. Разработка РКД (РРД)**  |  —  |**200‑215**  |
|·   3.1 Формирование РКД (РРД) на КК и его СЧ  |  СГК  |   185  |
|·   3.2 Проверка РКД (РРД) ГКП  |  ГКП  |   15  |
|·   3.3 *Доработка РКД (РРД), выпуск ИИ, проверка ГКП (при необх.)*  |  СГК  |   15  |
|**4. Визирование материалов ВП**  |  —  |**17‑48**  |
|·   4.1 Сборка и отправка в ВП документов письмом   |  СГК  |   2  |
|·   4.2 Рассмотрение и визирование материалов РКД (РРД)  |  ВП  |   15  |
|·   4.3 *При отклонении:*  |  —  |   31  |
|··     *4.3.1 Доработка РКД (РРД), выпуск ИИ, проверка ГКП*  |  СГК, ГКП  |      *15*  |
|··     *4.3.2 Сборка и отправка в ВП документов письмом*  |  СГК  |      *1*  |
|··     *4.3.3 Рассмотрение и визирование материалов РКД (РРД)*  |  ВП  |      *15*  |
|**5. Закрытие этапа РКД (РРД)**  |  —  |**45**  |
|·   5.1 Размножение док‑ов для передачи изготовителю *(параллельно)*  |  СГК  |   5  |
|·   5.2 Закрытие актов смежникам  |  СГК  |   20  |
|·   5.3 Закрытия этапа у Заказчика  |  Дир.  |   25  |
|• • •|||
|  **Итого:**| 432 ‑ 496 к.д.,<br> 14.4 ‑ 16.5 мес.  |**308 ‑ 354**  |
